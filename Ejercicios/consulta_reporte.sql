...
-- ======= ACÁ VA EL QUERY ========
--Solución con JOIN sin WHERE
SELECT
  v.id AS viaje_id,
  (SELECT COUNT(1) FROM paquetes p WHERE p.viaje_id = v.id) AS cantidad_paquetes,
  SUM(t.subtotal) AS total_tarifas
FROM
  viajes v
JOIN
  tarifas t ON v.id = t.viaje_id
GROUP BY
  v.id
HAVING
  cantidad_paquetes <= 3  
...
--Otra solución sin JOIN con WHERE
SELECT
  v.id AS viaje_id,
  (SELECT COUNT(1) FROM paquetes p WHERE p.viaje_id = v.id) AS cantidad_paquetes,
  SUM(t.subtotal) AS total_tarifas
